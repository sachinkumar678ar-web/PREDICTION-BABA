...
async function fetchData() {
    const proxies = [
        ...
        API_URL // Direct hit as last resort
    ];

    for (let url of proxies) {
        try {
            const res = await axios.get(url, { timeout: 6000 });
            if (res.data && res.data.data && res.data.data.list) return res.data.data.list;
        } catch (e) { continue; }
    }
    return null;
}
...
bot.command("members", (ctx) => {
    ...
    if (!activeMembers.has(userId)) {
        activeMembers.set(userId, { memberId: Math.floor(10000000 + Math.random() * 90000000), status: "inactive", expiry: null });
    }
...
bot.action(/p_(\d+)_(\d+)/, async (ctx) => {
    ...
    if (!tUser) return ctx.answerCbQuery("ID not found! User must send /members first.");
    
    const expiry = Date.now() + (mins * 60 * 1000);
    activeMembers.set(tUser, { ...activeMembers.get(tUser), status: "active", expiry });
...
setInterval(() => axios.get(SELF_URL).catch(() => {}), 120000); // 2 min ping
setInterval(scan, 8000); // Fast 8s scan

bot.launch();
const app = express();
app.get("/", (r, s) => s.send("Bot is Live with No Firebase"));
